#!/usr/bin/env bash
# Generate the Parquet datasets used by the column-rs DuckDB parity benchmark.
#
# Ported from column-rs's own benches/data/generate.sh (github.com/
# iheitlager/column-rs, #100) into this repo's parity/column-rs harness.
#
...
#   ./data/generate.sh xlarge       # opt-in, ~10GB, slow
#
# Pinned DuckDB version: v1.5.5 (Variegata) -- matching the version
# column-rs's own tests/oracle.rs pins, so generated data is
...
#
# The table name column-rs uses is the file stem, so queries in ../queries/

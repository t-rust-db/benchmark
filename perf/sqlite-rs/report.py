#!/usr/bin/env python3
"""Assembles results/bench-status.json from the raw tier-1 (criterion) and
tier-2 (hyperfine) outputs `make bench`/`make bench-cli` already produced
(ported from sqlite-rs's tools/bench_status.py, t-rust-db/sqlite-rs#22).
...
SQLITE_RS_REPO = Path(
    os.environ.get("SQLITE_RS_REPO", REPO_ROOT / "../../../sqlite-rs")
).resolve()
CRITERION_DIR = REPO_ROOT / "target" / "criterion"
HYPERFINE_DIR = Path(
    os.environ.get("BENCH_FIXTURES_DIR", REPO_ROOT / "target" / "bench-fixtures")
) / "hyperfine"
OUT_PATH = REPO_ROOT / "results" / "bench-status.json"
...
def pinned_oracle_version():
    cargo_toml = tomllib.loads((SQLITE_RS_REPO / "Cargo.toml").read_text())

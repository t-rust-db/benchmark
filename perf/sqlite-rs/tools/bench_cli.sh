...
if [ ! -f "$BENCH_DIR/bench_1mb.db" ] || [ ! -f "$BENCH_DIR/bench_50mb.db" ]; then
  ...
  "$SQLITE_RS_REPO/tools/gen_fixtures.sh" --bench
fi

# The binary under test, resolved like parity/sqlite-rs: $SQLITE_RS, then
# `sqlite-rs` on PATH, then a release build in the sqlite-rs checkout (built
# there if missing).
if [ -z "${SQLITE_RS:-}" ]; then
  if command -v sqlite-rs >/dev/null 2>&1; then
    SQLITE_RS="$(command -v sqlite-rs)"
  else
    SQLITE_RS="$SQLITE_RS_REPO/target/release/sqlite-rs"
    [ -x "$SQLITE_RS" ] || {
      echo "building sqlite-rs (release) in $SQLITE_RS_REPO..." >&2
      (cd "$SQLITE_RS_REPO" && cargo build --release --bin sqlite-rs --quiet)
    }
  fi
fi
